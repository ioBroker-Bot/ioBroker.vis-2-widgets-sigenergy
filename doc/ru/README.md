...
Содержит 7 виджетов для визуализации и управления потоком энергии, состоянием аккумулятора, мощностью в реальном времени, дневной статистикой, AC-зарядным устройством, DC-зарядным устройством и инвертором.
...
Установить адаптер через ioBroker Admin в виде ZIP-файла:

1. Admin → Адаптеры → «Установить из собственного URL» (значок GitHub)
2. Загрузить ZIP-файл или указать URL
3. Дождаться завершения установки — VIS-2 перезапустится автоматически

> **Примечание:** После установки необходимо **перезагрузить редактор VIS-2 в браузере**
> (F5 или обновить страницу), чтобы виджеты появились в палитре.
> Адаптер VIS-2 перезапускается автоматически, однако браузер необходимо
> обновить вручную.
...
### 1.3.2 (2026-03-12)
* Добавлена немецкая документация в doc/de/README.md
* README: добавлен раздел документации со ссылками на языки
